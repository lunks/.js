...
if (document.location.href.match(/github.com\/?$/)) {
  ...
  if (last) {
    ...
    $('div.news div.alert').each(function() {
      console.debug($(this).find('abbr.relatize').attr('title'), hit_last)
      if ($(this).find('abbr.relatize').attr('title') == last) {
        hit_last = true
      }
  ...
  localStorage.setItem('last_alert', $('div.news div.alert:first').find('abbr.relatize').attr('title'))
}
$(".sponsor").remove();
$(".locales .site").remove();
$("#footer .links").remove();
